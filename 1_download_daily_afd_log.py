...
def lambda_handler(event, context):
    ...
    afd_homepage = scraping.download_html('Wikipedia:Articles_for_deletion',
                                          wikipedia_prefix)
        
    # store afd page to  to s3/afd-scraped/Articles_for_deletion/[date].txt
    scraping.store_html(afd_homepage, 'Wikipedia:Articles_for_deletion')
    ...
    # download daily afd logs(i.e. pages in the format
    # https://en.wikipedia.org/wiki/Wikipedia:Articles_for_deletion/Log/2019_May_22)
    daily_afd = scraping.generate_df_of_daily_logs(afd_homepage_links)
    daily_afd['response'] = daily_afd['log_url'].apply(lambda x:
                                                     scraping.download_html(x,
                                                                            wikipedia_domain))
  
    # store the daily afd logs at
    # afd-scraped/daily_afd_log/[scraping-date]/[AFD log date].txt
    daily_afd[['response', 'log_url']].apply(lambda x:
                                           scraping.store_html(x[0],
                                                               'daily_afd_log',
                                                               fileName=x[1].replace("/wiki/Wikipedia:Articles_for_deletion/Log/", "")),
                                           axis=1)
